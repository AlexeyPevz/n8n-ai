...
export async function registerPlannerRoutes(server) {
    ...
    server.post('/ai/recommend-model', async (req) => {
        ...
        const recommendation = ModelSelector.recommend ? ModelSelector.recommend(prompt) : 'gpt-4o-mini';
        const requirements = ModelSelector.analyzePrompt ? ModelSelector.analyzePrompt(prompt) : { latency: 'medium', context: 'short' };
        return {
            recommendation,
            requirements,
