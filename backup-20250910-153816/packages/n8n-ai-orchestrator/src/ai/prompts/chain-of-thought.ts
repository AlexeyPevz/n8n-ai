...
export class ChainOfThoughtPlanner {
   ...
   */
  static decomposeRequest(prompt: string): ThoughtStep[] {
   ...
   */
  static generateReasoning(steps: ThoughtStep[]): string {
    const reasoning = [
      '## Workflow Analysis\n',
   ...
   */
  static planToOperations(plan: WorkflowPlan): OperationBatch {
   ...
   */
  private static extractActions(prompt: string): string[] {
    ...
    if (actions.length === 0) {
      if (promptLower.includes('api')) actions.push('fetch');
      if (promptLower.includes('database')) actions.push('query');
      if (promptLower.includes('email')) actions.push('send');
      if (promptLower.includes('file')) actions.push('process');
    }
   ...
   */
  private static selectTrigger(prompt: string): string {
    const promptLower = prompt.toLowerCase();
   ...
   */
  private static getTriggerNode(prompt: string): string {
    const promptLower = prompt.toLowerCase();
    
    if (promptLower.includes('webhook')) return 'n8n-nodes-base.webhook';
    if (promptLower.includes('schedule') || promptLower.includes('every')) return 'n8n-nodes-base.scheduleTrigger';
    if (promptLower.includes('email') && promptLower.includes('receive')) return 'n8n-nodes-base.emailTrigger';
    if (promptLower.includes('form')) return 'n8n-nodes-base.formTrigger';
   ...
   */
  private static createStepForAction(action: string, index: number): ThoughtStep {
    const actionLower = action.toLowerCase();
    
    const actionMappings: Record<string, ThoughtStep> = {
      fetch: {
        title: 'Fetch Data',
   ...
   * Generate node name
   */
  private static generateNodeName(nodeType: string, stepIndex: number, nodeIndex: number): string {
   ...
   * Get node version
   */
  private static getNodeVersion(nodeType: string): number {
   ...
   */
  private static getDefaultParameters(nodeType: string): Record<string, any> {
   ...
   */
  private static inferConnections(steps: ThoughtStep[]): Array<{ from: string; to: string }> {
