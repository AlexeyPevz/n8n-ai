 ...
 */
import { NodeSchema, OperationBatchSchema } from '@n8n-ai/schemas';
            ...
            for (const op of batch.ops) {
                switch (op.op) {
                    ...
                    case 'connect': {
                        ...
                        const existingConnection = workflow.connections.find(c => c.from === op.from && c.to === op.to && c.index === (op.index ?? 0));
                    ...
                    case 'delete': {
                        ...
                        workflow.connections = workflow.connections.filter(c => c.from !== nodeName && c.to !== nodeName &&
    ...
    hasCycles(workflow) {
        ...
        const dfs = (nodeId) => {
            ...
            for (const conn of outgoing) {
                const targetNode = workflow.nodes.find(n => n.id === conn.to || n.name === conn.to);
                if (!targetNode)
                    continue;
