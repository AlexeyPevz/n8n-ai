...
export function createAIRoutes(): Router {
    ...
    router.use((req: Request & { startTime?: number }, _res: Response, next: NextFunction) => {
    ...
    router.get('/introspect/nodes', (_req: Request, res: Response) => {
        try {
    ...
    router.get('/introspect/node/:type', (req: Request, res: Response) => {
        try {
    ...
    router.post('/graph/:id/batch', async (req, res) => {
        try {
            const { id } = req.params;
            let parsed: { success: true; data: unknown } | { success: false; error: { format: () => unknown } } = { success: true, data: req.body };
            try {
            ...
            try {
                ...
                return res.json({ ok: true, workflowId: id, appliedOperations: (batch as { ops: unknown[] }).ops.length, undoId, note: 'fallback_in_memory' });
            }
        }
        catch (error: unknown) {
    ...
    router.post('/graph/:id/undo', (req, res) => {
        try {
            ...
            let entry;
            if (undoId) {
                const idx = stack.findIndex((e: { undoId: string }) => e.undoId === undoId);
                if (idx === -1)
    ...
    router.use((req: Request & { startTime?: number }, res: Response, next: NextFunction) => {
