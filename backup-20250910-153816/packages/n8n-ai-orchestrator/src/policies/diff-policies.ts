...
export type DiffPolicy = z.infer<typeof DiffPolicySchema>;

// Policy implementations
export interface PolicyChecker {
  check(batch: OperationBatch, context: PolicyContext): void;
}
...
export class NodeWhitelistPolicy implements PolicyChecker {
  constructor(private config: z.infer<typeof NodeWhitelistPolicySchema>) {}
        ...
        if (!this.config.whitelist.includes(nodeType)) {
          if (!this.config.allowUnknown) {
            throw new PolicyViolationError(
              'node_whitelist',
              `Node type '${nodeType}' is not whitelisted`,
              { nodeType, whitelist: this.config.whitelist }
            );
          }
        }
      }
...
export class OperationLimitPolicy implements PolicyChecker {
  constructor(private config: z.infer<typeof OperationLimitPolicySchema>) {}
  ...
  check(batch: OperationBatch, context: PolicyContext): void {
    ...
    if (batch.ops.length > this.config.maxOperations) {
      throw new PolicyViolationError(
        'operation_limit',
        `Batch contains ${batch.ops.length} operations, exceeding limit of ${this.config.maxOperations}`,
        { operationCount: batch.ops.length, limit: this.config.maxOperations }
      );
    }
    ...
    if (this.config.maxNodesPerBatch) {
      ...
      if (nodeOps > this.config.maxNodesPerBatch) {
        throw new PolicyViolationError(
          'operation_limit',
          `Batch adds ${nodeOps} nodes, exceeding limit of ${this.config.maxNodesPerBatch}`,
          { nodeCount: nodeOps, limit: this.config.maxNodesPerBatch }
        );
      }
    }
    ...
    if (this.config.maxConnectionsPerBatch) {
      ...
      if (connectionOps > this.config.maxConnectionsPerBatch) {
        throw new PolicyViolationError(
          'operation_limit',
          `Batch creates ${connectionOps} connections, exceeding limit of ${this.config.maxConnectionsPerBatch}`,
          { connectionCount: connectionOps, limit: this.config.maxConnectionsPerBatch }
        );
      }
    }
  }
...
export class NodeTypeLimitPolicy implements PolicyChecker {
  constructor(private config: z.infer<typeof NodeTypeLimitPolicySchema>) {}
  ...
  check(batch: OperationBatch, context: PolicyContext): void {
    ...
    for (const [nodeType, limit] of Object.entries(this.config.limits)) {
      ...
      if (total > limit) {
        throw new PolicyViolationError(
          'node_type_limit',
          `Total ${nodeType} nodes would be ${total}, exceeding limit of ${limit}`,
          { nodeType, existing, adding, total, limit }
        );
      }
    }
  }
...
export class ParameterPolicy implements PolicyChecker {
  constructor(private config: z.infer<typeof ParameterPolicySchema>) {}
  ...
  check(batch: OperationBatch, context: PolicyContext): void {
    ...
    for (const op of batch.ops) {
      if (op.op === 'add_node' || op.op === 'set_params') {
        const nodeType = op.op === 'add_node' ? op.node.type : this.getNodeType(((op as any).nodeId || (op as any).name), context);
        const params = op.op === 'add_node' ? op.node.parameters : ((op as any).parameters || (op as any).params);
  ...
  private checkRule(rule: any, params: any, nodeType: string): void {
    ...
    switch (rule.policy) {
      case 'forbidden':
        if (value !== undefined) {
          throw new PolicyViolationError(
            'parameter_policy',
            rule.message || `Parameter '${rule.parameter}' is forbidden for ${nodeType}`,
            { nodeType, parameter: rule.parameter, value }
          );
        }
        break;
      ...
      case 'required':
        if (value === undefined) {
          throw new PolicyViolationError(
            'parameter_policy',
            rule.message || `Parameter '${rule.parameter}' is required for ${nodeType}`,
            { nodeType, parameter: rule.parameter }
          );
        }
        break;
      ...
      case 'pattern':
        if (value !== undefined && rule.pattern) {
          ...
          if (!regex.test(String(value))) {
            throw new PolicyViolationError(
              'parameter_policy',
              rule.message || `Parameter '${rule.parameter}' does not match required pattern`,
              { nodeType, parameter: rule.parameter, value, pattern: rule.pattern }
            );
          }
        }
        break;
...
export class CostLimitPolicy implements PolicyChecker {
  constructor(private config: z.infer<typeof CostLimitPolicySchema>) {}
  ...
  check(batch: OperationBatch, context: PolicyContext): void {
    ...
    if (context.estimatedCost !== undefined) {
      if (context.estimatedCost > this.config.maxEstimatedCost) {
        throw new PolicyViolationError(
          'cost_limit',
          `Estimated cost ${context.estimatedCost} ${this.config.costUnit} exceeds limit of ${this.config.maxEstimatedCost}`,
          { 
...
export class WorkflowComplexityPolicy implements PolicyChecker {
  constructor(private config: z.infer<typeof WorkflowComplexityPolicySchema>) {}
  ...
  check(batch: OperationBatch, context: PolicyContext): void {
    ...
    if (nodeCount > this.config.maxNodes) {
      throw new PolicyViolationError(
        'workflow_complexity',
        `Workflow would have ${nodeCount} nodes, exceeding limit of ${this.config.maxNodes}`,
        { nodeCount, limit: this.config.maxNodes }
      );
    }
    ...
    if (connectionCount > this.config.maxConnections) {
      throw new PolicyViolationError(
        'workflow_complexity',
        `Workflow would have ${connectionCount} connections, exceeding limit of ${this.config.maxConnections}`,
        { connectionCount, limit: this.config.maxConnections }
      );
    }
...
export class DiffPolicyManager {
  private policies: PolicyChecker[] = [];
  
  constructor(configs: DiffPolicy[]) {
    for (const config of configs) {
      this.policies.push(this.createPolicy(config));
    }
  }
  
  private createPolicy(config: DiffPolicy): PolicyChecker {
    switch (config.type) {
      case 'node_whitelist':
      ...
      default:
        throw new Error(`Unknown policy type: ${(config as any).type}`);
    }
  }
  
  checkBatch(batch: OperationBatch, context: PolicyContext): void {
    for (const policy of this.policies) {
      policy.check(batch, context);
    }
  }
  
  async checkBatchAsync(batch: OperationBatch, context: PolicyContext): Promise<void> {
