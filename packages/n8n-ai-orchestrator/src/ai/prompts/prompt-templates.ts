...
export class PromptEngineer {
  ...
  static selectTemplate(
    context: {
      ...
      requestType?: 'create' | 'modify' | 'fix' | 'optimize';
      hasErrors?: boolean;
    },
    templateType: 'system' | 'planner' | 'critic'
  ): string {
   ...
   */
  static formatReasoning(steps: Array<{ title: string; content: string }>): string {
    return steps
   ...
   */
  static analyzeComplexity(prompt: string): 'simple' | 'medium' | 'complex' {
    const indicators = {
      complex: [
    ...
    if (complexCount >= 2 || prompt.length > 200) return 'complex';
    if (mediumCount >= 2 || complexCount >= 1) return 'medium';
    return 'simple';
  }
}