...
export class GitIntegration {
     ...
     */
    async createCommit(workflowId, workflowName, batch, context) {
        try {
            ...
            const workflowFile = path.join(this.config.repoPath, 'workflows', `${workflowId}.json`);
            ...
            const commitMessage = this.buildCommitMessage(workflowName, batch, context);
            // Set author
            await execAsync(`git -c user.name="${this.config.author.name}" ` +
            ...
            if (this.config.pullRequest?.enabled) {
                pullRequestUrl = await this.createPullRequest(branchName, workflowName, batch, context);
            }
            // Generate diff URL
     ...
     */
    buildCommitMessage(workflowName, batch, context) {
     ...
     */
    async createPullRequest(branchName, workflowName, batch, context) {
        ...
        try {
            // Get repository info
            const { stdout: remoteUrl } = await execAsync(`git config --get remote.${this.config.remote}.url`);
            ...
            const { stdout: prUrl } = await execAsync(`gh pr create ` +
                `--title "${prTitle}" ` +
                `--body "${prBody}" ` +
     ...
     */
    buildPullRequestBody(workflowName, batch, context) {
     ...
     */
    async getUpdatedWorkflow(workflowId) {
