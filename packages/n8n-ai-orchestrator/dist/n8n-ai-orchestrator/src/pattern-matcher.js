    ...
    findMatchingPatterns(prompt) {
        ...
        for (const pattern of this.patterns) {
            ...
            for (const node of pattern.nodes) {
                ...
                if (promptLower.includes('slack') && nodeType.includes('slack'))
                    score += 2;
                if (promptLower.includes('email') && nodeType.includes('email'))
                    score += 2;
                if (promptLower.includes('database') && (nodeType.includes('postgres') || nodeType.includes('mysql')))
                    score += 2;
                if (promptLower.includes('sheet') && nodeType.includes('googlesheet'))
                    score += 2;
                if (promptLower.includes('ai') && nodeType.includes('langchain'))
                    score += 3;
            }
            if (score > 0) {
                results.push({
