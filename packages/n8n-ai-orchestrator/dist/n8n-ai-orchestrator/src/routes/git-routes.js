...
export async function registerGitRoutes(server, _options) {
    ...
    server.post('/api/v1/git/commit', {
        ...
        handler: async (request, reply) => {
            ...
            try {
                // Create commit
                const result = await gitIntegration.commitWorkflow({ id: body.workflowId, name: body.workflowName }, body.message || 'AI-generated workflow update', {
                    userId: request.headers['x-user-id'],
                    promptUsed: body.prompt,
                    description: body.description,
                });
                // Log to audit
                await auditLogger.logOperation(body.operationBatch, {
    ...
    server.post('/api/v1/git/validate', {
        handler: async (request, reply) => {
            ...
            const deleteOps = operationBatch.ops.filter((op) => op.op === 'delete');
            ...
            const hasCredentials = operationBatch.ops.some((op) => op.op === 'set_params' &&
                op.params &&
                Object.keys(op.params).some(key => key.toLowerCase().includes('credential') ||
