 ...
 */
export function findMatchingPattern(prompt) {
    const promptLower = prompt.toLowerCase();
    for (const pattern of WORKFLOW_PATTERNS) {
        const matchCount = pattern.keywords.filter(keyword => promptLower.includes(keyword)).length;
...
export function generateOperationsFromPattern(pattern) {
    const operations = [];
    // Добавляем ноды
