...
export async function registerMetricsRoutes(server: FastifyInstance) {
  ...
  server.get('/metrics/history', async (request, reply) => {
    ...
    return metricsDashboard.getHistoricalMetrics(
  ...
  server.get('/metrics/range/:range', async (request, reply) => {
    const { range } = request.params as { range: '1h' | '6h' | '24h' | '7d' };
...
export async function registerDashboardRoute(server: FastifyInstance) {
